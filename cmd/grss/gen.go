...
type gen struct {
	...
	SocksUsername   string `long:"user" description:"SOCKS5 username"`    // 新增
	SocksPassword   string `long:"pass" description:"SOCKS5 password"`    // 新增
	Positional      struct {
...
func (c *gen) Execute(args []string) error {
	...
	// 新增: 校验用户名和密码
	if (c.SocksUsername != "" && c.SocksPassword == "") || (c.SocksUsername == "" && c.SocksPassword != "") {
		return errors.New("both username and password must be provided for SOCKS5 authentication")
	}

	clientConfig := config.ToClientConfig(0)
	// 新增: 将用户名和密码添加到客户端配置中
	clientConfig.SocksUsername = c.SocksUsername
	clientConfig.SocksPassword = c.SocksPassword

	return c.genClient(clientConfig)
}
