...
func main() {
	...
	if config.SocksUsername != "" && config.SocksPassword != "" {
		creds := socks5.StaticCredentials{
			config.SocksUsername: config.SocksPassword,
		}
		authMethods = append(authMethods, socks5.UserPassAuthenticator{Credentials: creds})
		logger.Info("SOCKS5 authentication enabled")
	}

	socksServer, err := socks5.New(&socks5.Config{AuthMethods: authMethods})
	if err != nil {
		logger.Fatalln(err)
	}
